...
public sealed record BLMReportGroup((int NumberOfMachines, double R) Key, IEnumerable<ExecutionReport> Reports)
{
    ...
    public int Variance => Reports.Sum(r => r.Variance) / Reports.Count();
    public int OptimumMakeSpan => Reports.Sum(r => r.OptimumMakeSpan) / Reports.Count();
    public int InstanceTotalMakeSpan => Reports.Sum(r => r.InstanceTotalMakeSpan) / Reports.Count();
    public int OptimumMinusMakeSpan => Reports.Sum(r => r.OptimumMinusMakeSpan) / Reports.Count();
